...
#   + 함수 호출시 입력 매개변수()와 함수 정의시 입력 매개변수는 동일한 갯수여야 함
#   $ 함수 호출 후 함수 종료 된 경우 "호출  했던 곳"으로 돌아감
#   $ 입력 매개변수의 순서가 매우 중요
#   $ 최초에는 함수 호출문으로 동작하지만, 함수 종료 후에는 변수로서 활용 됨
#     반환값이 있는 경우 반환값을 담는 변수, 변환값이 없는 경우 None
print(sum_two_value(5,10))

#  4. 인자 
#   + 매개변수 = parameter = arguemnt
#   + 함수에 전달되는 입력값
...
#   def test(list_val): -> (O)
#   코드 작성시 종속서은 최대한 배제하면서 개발!
#      -> 종속성: A 코드를 수정하면 B 코드, C코드도 수정해야하는 경우
...
#  8. 가변 길이 인자(Variable Length Parameter)
